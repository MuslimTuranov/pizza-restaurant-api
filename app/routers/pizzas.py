...
def create_pizza(data: schemas.PizzaCreate, db: Session = Depends(get_db)):
    ...
    ingredients = db.query(models.Ingredient).filter(models.Ingredient.id.in_(data.ingredient_ids)).all()
...
def update_pizza(pizza_id: int, data: schemas.PizzaUpdate, db: Session = Depends(get_db)):
